...
def load_all_data(file_path):
    try:
        ...
        prog_map = df_courses[['學程代碼', '學程名稱']].drop_duplicates().set_index('學程代碼')['學名稱'].to_dict()
...
# --- TAB 3: 修改重點 - 全局比對 + 排序 + 模組化細節 ---
with tab_audit:
    st.header("🎓 學程達成度全局排行")
    
    c1, c2 = st.columns([1, 2])
    with c1: st.download_button("📥 下載成績單範本", data=get_template_excel(), file_name="範本.xlsx", use_container_width=True)
    ...
    if uploaded_file:
        try:
            ...
            # 4. 顯示結果
            st.info(f"💡 比對完成！共分析 {len(audit_results)} 個學程，已依達成率為您排序。")
                        ...
                        for mod_name in res['details']['模組名稱'].unique():
                            ...
                            st.markdown(f"""
                                <div class="module-summary-box">
                                    ...
                                    <span>已獲學分：{int(mod_done)}</span>
                                </div>
                            """, unsafe_allow_html=True)
                            
                            # 顯示該模組課程表格
                            sub_df = mod_data[['科目類別', '課程代碼', '課程名稱', '學分數', '已完成']].copy()
                            sub_df['狀態'] = sub_df['已完成'].apply(lambda x: "✅ 已達成" if x else "❌ 未達成")
